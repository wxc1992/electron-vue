<template>
  <div id="ratingUp">
      ...
      <ratingUpdateDilog :showDioRating="showDioRating" @sendClose="sendClose" :rowstatus="rowstatus"  @sendResult="sendResult"></ratingUpdateDilog>
      <div class="clearfix">
    ...
    <div :style="{height:tableHeight2+50+'px' }">
      <el-table
        ref="multipleTable2"
        :data="tableData"
        v-loading="loading"
        ...
        <el-table-column
          ...
          label="操作">
          <template slot-scope="scope">
            <a href="javascript:;" style="text-decoration: none;color: #409EFF" @click="showratingDio(scope.row)">详情</a>
          </template>
        </el-table-column>
      </el-table>
       <el-pagination
...
import ratingUpdateDilog from './ratingUpdateDilog.vue';
const maps = {
  5034: '城投',
  5035: '钢铁',
  5036: '有色',
...
export default {
  ...
  mounted() {
    ...
    const that = this;
    setTimeout(() => {
      const multipleTable2 = that.$refs.multipleTable2;
      if (!multipleTable2) {
        return;
      }
      const tableTop = this.$refs.multipleTable2.$el.getBoundingClientRect().top;
      const winHeight = window.innerHeight;
      this.tableHeight2 = window.innerHeight - 369;
    }, 200);
    this.$winResize.add(() => {
      ...
      that.delay = setTimeout(() => {
        ...
        const tableTop = that.$refs.multipleTable2.$el.getBoundingClientRect().top;
        const winHeight = window.innerHeight;
        this.tableHeight2 = window.innerHeight - 369;
      }, 200);
    });
  ...
  methods: {
    ...mapActions(['getDeepSubject', 'getFiltersNew', 'getmodCreditSubject', 'companyList', 'approvalcompany', 'getFivesheetData']),
    ...
    showratingDio(row) {
      ...
      this.rowstatus = row;
      this.showDioRating = true;
    },
    sendClose(val) {
      this.showDioRating = val;
    },
    sendResult(obj) {
    ...
    async rejectPass(arr) {
      ...
      const res = await this.approvalcompany(params);
      console.log('approvalcompany', res);
      if (res.status === 200) {
    ...
    async getompanyList() {
      ...
      const res = await this.companyList(params);
